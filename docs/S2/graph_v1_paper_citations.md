...
**接口路径：** `GET https://ai4scholar.net/graph/v1/paper/{paper_id}/citations`
...
获取引用此论文的论文详情（即在其参考文献中包含此论文的论文）

示例：

• 假设以下示例中的论文有 1600 条引用...
• `https://ai4scholar.net/graph/v1/paper/649def34f8be52c8b66281af98ae884c09aef38b/citations`

• 返回 offset=0, next=100，data 是包含 100 条引用的列表。
• 每条引用有一个 citingPaper，包含其 paperId 和 title。

• `https://ai4scholar.net/graph/v1/paper/649def34f8be52c8b66281af98ae884c09aef38b/citations?fields=contexts,intents,isInfluential,abstract&offset=200&limit=10`

• 返回 offset=200, next=210，data 是包含 10 条引用的列表。
• 每条引用有 contexts、intents、isInfluential 和一个 citingPaper（包含 paperId 和 abstract）。

• `https://ai4scholar.net/graph/v1/paper/649def34f8be52c8b66281af98ae884c09aef38b/citations?fields=authors&offset=1500&limit=500`

• 返回 offset=1500，data 是包含最后 100 条引用的列表。
• 每条引用有一个 citingPaper，包含 paperId 和作者列表。
• 每个 citingPaper 下的作者包含 authorId 和 name。
...
url = "https://ai4scholar.net/graph/v1/paper/{paper_id}/citations"
headers = {
    "x-api-key": API_KEY,
}
...
```bash
curl -X GET "https://ai4scholar.net/graph/v1/paper/{paper_id}/citations" \
  -H "x-api-key: YOUR_API_KEY"
```

## 响应

### 200 包含默认或请求字段的引用批量数据
...
| `citingPaper` | object | 关于引用论文的详细信息 |
