...
**接口路径：** `GET https://ai4scholar.net/graph/v1/paper/search/bulk`
...
与 `/paper/search` 类似，但用于批量检索基本论文数据，不考虑搜索相关性：

• 文本查询是可选的，支持布尔逻辑进行文档匹配。
• 可以使用各种条件筛选论文。
• 每次调用最多返回 1,000 篇论文。
• 如果有更多匹配的论文，将返回一个继续令牌（token）。
• 可以添加 token 参数重复查询，以高效继续获取匹配的论文。


返回包含估计总匹配数、匹配论文批次和继续令牌（如有更多结果）的结构。

限制：

• 此方法不支持嵌套论文数据，如引用、参考文献等。
• 此方法最多可获取 10,000,000 篇论文。如需更多数据，请使用数据集 API 获取完整语料库副本。
...
文本查询，将与论文的标题和摘要进行匹配。所有词项使用英文词干提取。默认情况下，查询中的所有词项必须出现在论文中。
...
• `*` 用于匹配前缀
• `(` 和 `)` 用于优先级
• `~N` 放在单词后匹配编辑距离为 N 的词（如省略 N 则默认为 2）
• `~N` 放在短语后匹配短语词项之间最多相隔 N 个词（如省略 N 则默认为 2）


示例：

• `fish ladder` 匹配包含 "fish" 和 "ladder" 的论文
• `fish -ladder` 匹配包含 "fish" 但不包含 "ladder" 的论文
• `fish | ladder` 匹配包含 "fish" 或 "ladder" 的论文
• `"fish ladder"` 匹配包含短语 "fish ladder" 的论文
• `(fish ladder) | outflow` 匹配包含 "fish" 和 "ladder" 或 "outflow" 的论文
• `fish~` 匹配包含 "fish"、"fist"、"fihs" 等的论文
• `"fish ladder"~3` 匹配包含短语 "fish ladder" 或 "fish is on a ladder" 的论文


#### `token`

用于分页。此字符串令牌在原始查询返回时提供，用于获取下一批论文。每次调用都会返回新的令牌。

#### `fields`

以逗号分隔的返回字段列表。请参阅下方响应架构中 `data` 数组的内容，了解所有可返回的字段。
`paperId` 字段始终返回。如果省略 fields 参数，则仅返回 `paperId` 和 `title`。
示例：`https://ai4scholar.net/graph/v1/paper/search/bulk?query=covid&fields=venue,s2FieldsOfStudy`
...
使用格式 `字段:顺序`。相同值按 `paperId` 排序。默认字段为 `paperId`，默认顺序为升序（asc）。排序值未定义的记录将出现在排序末尾，无论升序/降序。


示例：

• `publicationDate:asc` - 最早的论文优先返回。
• `citationCount:desc` - 引用最多的论文优先返回。
• `paperId` - 按 ID 顺序返回论文，从低到高。


请注意，如果在翻页过程中相关数据发生变化，记录可能以意外方式返回。默认的 `paperId` 排序可避免此边缘情况。
...
url = "https://ai4scholar.net/graph/v1/paper/search/bulk"
headers = {
    "x-api-key": API_KEY,
}
...
```bash
curl -X GET "https://ai4scholar.net/graph/v1/paper/search/bulk" \
...
| `token` | string | 必须提供以获取下一页结果的继续令牌。仅在可以获取更多结果时存在。 |
