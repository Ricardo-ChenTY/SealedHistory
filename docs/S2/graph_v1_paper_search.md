...
**接口路径：** `GET https://ai4scholar.net/graph/v1/paper/search`
...
• `https://ai4scholar.net/graph/v1/paper/search?query=covid+vaccination&offset=100&limit=3`

• 返回 total=576278, offset=100, next=103，data 是包含 3 篇论文的列表。
• 每篇论文包含 paperId 和 title。

• `https://ai4scholar.net/graph/v1/paper/search?query=covid&fields=url,abstract,authors`

• 返回 total=639637, offset=0, next=100，data 是包含 100 篇论文的列表。
• 每篇论文包含 paperId、url、abstract 和作者列表。
• 每个作者包含 authorId 和 name。

• `https://ai4scholar.net/graph/v1/paper/search?query=totalGarbageNonsense`

• 返回 total=0, offset=0，data 是空列表。

• `https://ai4scholar.net/graph/v1/paper/search?query=covid&year=2020-2023&openAccessPdf&fieldsOfStudy=Physics,Philosophy&fields=title,year,authors`

• 返回 total=8471, offset=0, next=10，data 是包含 10 篇论文的列表。
• 筛选仅包含 2020-2023 年发表的论文。
• 筛选仅包含有开放获取 PDF 的论文。
• 筛选仅包含研究领域为 Physics 或 Philosophy 的论文。
• 每篇论文包含 paperId、title、year 和 authors 字段。


限制：

• 最多返回 1,000 条相关性排名结果。对于更大的查询，请参阅 /search/bulk 或数据集 API。
• 一次最多返回 10 MB 数据。
...
示例：
`graph/v1/paper/search?query=generative ai`
